...
describe("FeishuConfigSchema #77116 regression (appId/appSecret/defaulted-required)", () => {
  // Reported issue: bundled channel config metadata generated by
  // scripts/generate-bundled-channel-config-metadata.ts marked
  // `.optional().default(...)` and `.default(...)` fields as JSON Schema
  // `required`. The reporter's config (omitting `domain`,
  // `connectionMode`, `webhookPath`, `reactionNotifications`,
  // `typingIndicator`, `resolveSenderNames`) was rejected even though every
  // omitted field has a documented default.
  const reporterConfig = {
    ...
    allowFrom: ["*"],
    accounts: {
      main: { appId: "cli_a9361beb52381bde", appSecret: "redacted" },
      finance: { appId: "cli_a934dc5be7389de", appSecret: "redacted" },
      research: { appId: "cli_a94b865a79b9dbb7", appSecret: "redacted" },
      spotlight: { appId: "cli_a948b11aeb395cc9", appSecret: "redacted" },
      learner: { appId: "cli_a94893e428395cef", appSecret: "redacted" },
      writer: { appId: "cli_a94fec6a2d385bc8", appSecret: "redacted" },
      default: {},
    },
  };

  it("accepts the reporter config via the runtime Zod parser", () => {
    const result = FeishuConfigSchema.safeParse(reporterConfig);
    expect(result.success).toBe(true);
  });

  it("does not list any Feishu properties as required in the generated JSON schema", () => {
    const built = buildChannelConfigSchema(FeishuConfigSchema);
    expect((built.schema.required as string[] | undefined) ?? []).toEqual([]);
  });

  it("accepts the reporter config via the generated JSON schema (applyDefaults: true)", () => {
    const built = buildChannelConfigSchema(FeishuConfigSchema);
    const result = validateJsonSchemaValue({
      schema: built.schema,
      cacheKey: "feishu-config-schema.test.json-validation",
      value: structuredClone(reporterConfig),
      applyDefaults: true,
      cache: false,
    });
  ...
  it("accepts the reporter config via the generated JSON schema even without defaults", () => {
    // With the #77116 fix, defaulted properties are no longer `required`, so
    // external tooling that does not apply defaults (e.g. packaged startup
    // validation in 2026.5.x crash-loop report) must also accept the config.
    const built = buildChannelConfigSchema(FeishuConfigSchema);
    const result = validateJsonSchemaValue({
      schema: built.schema,
      cacheKey: "feishu-config-schema.test.no-defaults",
      value: structuredClone(reporterConfig),
      applyDefaults: false,
      cache: false,
    });
