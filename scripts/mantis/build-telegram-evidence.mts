...
type TelegramEvidenceEntry = {
  execution?: {
    packageSource?: { kind?: string };
    provider?: { auth?: string; fixture?: string };
  };
  result?: {
    failure?: { reason?: string };
    status?: string;
    timing?: { p50Ms?: number; rttMs?: number };
  };
...
function formatMessageText(message: TelegramObservedMessage) {
...
function evidenceEntries(summary: unknown): TelegramEvidenceEntry[] {
  if (!summary || typeof summary !== "object") {
    return [];
  }
  const entries = Reflect.get(summary, "entries");
  return Array.isArray(entries) ? entries : [];
}

function evidenceCounts(summary: unknown) {
...
function evidenceCredentialSource(summary: unknown) {
...
function renderScenarioList(summary: unknown) {
  const entries = evidenceEntries(summary);
  if (entries.length === 0) {
...
function renderObservedMessages(observedMessages: unknown) {
  ...
  const messages: TelegramObservedMessage[] = observedMessages;
  return messages
    .map((message, index) => {
...
export function renderTelegramEvidenceHtml({
  ...
  const pass = counts.failed === 0 && counts.total > 0;
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
export function buildTelegramEvidenceManifest({
  ...
  candidateRef?: string;
  candidateSha?: string;
  hasObservedMessages?: boolean;
  scenarioLabel?: string;
  summary: unknown;
  summaryArtifactPath?: string;
}): TelegramEvidenceManifest {
  const counts = evidenceCounts(summary);
  const pass = counts.failed === 0 && counts.total > 0;
...
export function writeTelegramEvidence(rawArgs: string[] = process.argv.slice(2)): {
  manifest: TelegramEvidenceManifest;
  manifestPath: string;
