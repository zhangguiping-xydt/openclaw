...
async function verifyCodexNativeSubagentBridgeProbe(params: {
  ...
  function listCodexNativeTasks() {
    return listTaskRecords().filter(
      (entry) =>
        entry.runtime === "subagent" &&
        entry.taskKind === "codex-native" &&
        entry.requesterSessionKey === params.sessionKey &&
        entry.createdAt >= probeStartedAt,
    );
  }

  async function waitForDeliveredCodexNativeTask(token: string) {
    while (Date.now() < deadline) {
      const tasks = listCodexNativeTasks();
    ...
    throw new Error(
      `expected delivered Codex-native task containing ${token}; initialText=${JSON.stringify(
        text,
      )}; events=${JSON.stringify(events)}; tasks=${JSON.stringify(listCodexNativeTasks())}`,
    );
  }
}
